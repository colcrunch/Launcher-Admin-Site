...
function checkpass($id, $password, $mysqli){
  if($stmt = $mysqli->prepare("SELECT id, username, email, password, salt FROM members WHERE id = ? LIMIT 1")){
...
function updatepass($id, $password, $mysqli){
  if($stmt = $mysqli->prepare("SELECT salt FROM members WHERE id = ? LIMIT 1")){
    ...
    $password = hash('sha512', $password . $salt);
    if($stmt->num_rows == 1){
      if($stt = $mysqli->prepare("UPDATE members SET password = ? WHERE id = ?")){
        $stt->bind_param('ss', $password, $id);
        $stt->execute();
